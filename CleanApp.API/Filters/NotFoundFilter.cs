...
public class NotFoundFilter<T,TId>(IGenericRepository<T,TId> genericRepository) : Attribute, IAsyncActionFilter where T : class where TId : struct
{
	...
	{
		var idValue = context.ActionArguments.TryGetValue("id", out var isAdObject) ? isAdObject : null;

		
		if (isAdObject is not TId id)
		{
			await next();
			return;
		}
		...
		var result = ServiceResult.Fail($"data was not found. ({entityName})({actionName})");
