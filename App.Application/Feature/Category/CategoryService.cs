...
public class CategoryService(ICategoryRepository categoryRepository, IUnitOfWork unitOfWork, IMapper mapper) : ICategoryService
{
	...
	{
		var anyCategory = await categoryRepository.AnyAsync(p => p.Name.ToLower() == createCategoryRequest.Name.ToLower());
	...
	public async Task<ServiceResult> UpdateAsync(int id, UpdateCategoryRequest updateCategoryRequest)
	{
		...
		var anyCategory = await categoryRepository.AnyAsync(p => p.Name.ToLower() == updateCategoryRequest.Name.ToLower() && p.Id != id);
